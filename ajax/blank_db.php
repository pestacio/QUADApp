				...
				<div>
					...
					<div class="widget-body">
						...
						<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
							<thead>			                
                                                                <tr>
                                                                    <th><?php echo $ui_code;?></th>
                                                                    <th data-hide="tablet"><?php echo $ui_designation;?></th>
                                                                    <th data-hide="tablet"><?php echo $ui_description;?></th>
                                                                    <th data-hide="tablet"><?php echo $ui_dt_begin;?></th>
                                                                    <th data-hide="tablet"><?php echo $ui_dt_end;?></th>
                                                                </tr>                                                                
							</thead>
<?php
 #               $stid = oci_parse($conn, 'SELECT CD_ARTIGO, DT_INI_ARTG, DSP_ARTIGO, DESCRICAO, DT_FIM FROM PRS_ARTIGOS ORDER BY 1');
 #               oci_execute($stid);
 #               while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) {                
 #                   echo "<tr>";
 #                   echo "<td>".$row['CD_ARTIGO']."</td>";
 #                  echo "<td>".$row['DSP_ARTIGO']."</td>";
 #                  echo "<td>".$row['DESCRICAO']."</td>";
 #                  echo "<td>".$row['DT_INI_ARTG']."</td>";
 #                  echo "<td>".$row['DT_FIM']."</td>";
 #                  echo "</tr>";
 #              }

    $db = new \Oracle\Db("Artigos", "PTE");
    $sql = "SELECT CD_ARTIGO, DT_INI_ARTG, DSP_ARTIGO, DESCRICAO, DT_FIM FROM PRS_ARTIGOS ORDER BY 1";
#    $c1 = 1;
#    $c2 = 'abc';
...
#                                   array(":c2_bv", $c2, -1)));
    ...
    $res = $db->execFetchAll($sql, "Artigos");
    foreach ($res as $row) {
            echo "<tr>";
            echo "<td>".$row['CD_ARTIGO']."</td>";
            echo "<td>".$row['DSP_ARTIGO']."</td>";
