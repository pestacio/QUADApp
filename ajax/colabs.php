						...
						<table id="colabs" class="table table-striped table-bordered table-hover" width="100%">
							<thead>			                
                                                                <tr>
                                                                    <th><?php echo 'RHId';?></th>
                                                                    <th><?php echo 'Nome';?></th>
                                                                    <th data-hide="tablet"><?php echo 'Foto';?></th>
                                                                </tr>                                                                
							</thead>
<?php
    ...
    $sql = "SELECT A.RHID,A.NOME FROM RH_IDENTIFICACOES A WHERE RHID IN (SELECT RHID FROM RH_ID_IMAGENS) ORDER BY 1";
    $res = $db->execFetchAll($sql, "Colaboradores");
    foreach ($res as $row) {
        
   #     $lob = get_foto($row['RHID']);
 #       if (is_object($row['FOTO'])) {
 #           $lob = $row['FOTO']->load();
 #           $row['FOTO']->free();
 #       }
        ...
        echo "<td>".$row['RHID']."</td>";
        echo "<td>".$row['NOME']."</td>";
        #$ficheiro =  "http://www.quad-systems.com/SmartAdmin/img/fotos/".$row['RHID'].".jpg";
        $ficheiro_url =  "http://www.quad-systems.pt/QUAD_PRS_DSV/img/fotos/".$row['RHID'].".jpg";
        $ficheiro_local = "/home/quadsysp/public_html/QUAD_PRS_DSV/img/fotos/".$row['RHID'].".jpg"; 
        
        if (file_exists($ficheiro_local))
            echo "<td><img width=30 heigth=40 src='".$ficheiro_url."'/></td>";
        else
            echo "<td>".$ficheiro_local."</td>";
        
#        echo "<td><img src='http://www.quad-systems.com/SmartAdmin/#ajax/foto.php?id=".$row['RHID']."'/></td>";
#        echo "<td><embed width=200 height=200 src='http://www.quad-systems.com/SmartAdmin/#ajax/foto.php?id=".$row['RHID']."' type='image/tiff' negative=yes>"</td>";


        
#echo "<td><img src='data:image/jpeg;base64,".base64_encode($lob)."' /></td>";
        echo "</tr>";
    }
